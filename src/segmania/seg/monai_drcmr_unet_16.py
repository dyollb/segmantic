...
if __name__ == "__main__":
    ...
    root_dir = "F:\\Data\\DRCMR-Thielscher\\all_data"

    num_classes = 16
    image_dir = r"F:\Data\DRCMR-Thielscher\all_data\mixed_images" #os.path.join(root_dir, "images")
    labels_dir = r"F:\Data\DRCMR-Thielscher\all_data\mixed_labels" #os.path.join(root_dir, "labels_%d" % num_classes)

    results_dir = os.path.join(
        "F:\\Data\\DRCMR-Thielscher\\monai", "mixed_labels_%d" % num_classes
    )
    os.makedirs(results_dir, exist_ok=True)
    log_dir = os.path.join(results_dir, "logs")
    model_file = os.path.join(results_dir, "drcmr_%d.ckpt" % num_classes)

    tissue_dict = load_tissue_list(
        os.path.join(labels_dir, "labels_%d.txt" % num_classes)
    )
    ...
    if args.predict:
        predict(
            ...
            #test_images=get_nifti_files(image_dir)[-6:],
            #test_labels=get_nifti_files(labels_dir)[-6:],
            #test_labels=[os.path.join(labels_dir, "X99815.nii.gz")],
            test_images=get_nifti_files("F:\\Data\\HeadSeg_Data\\T1"),
